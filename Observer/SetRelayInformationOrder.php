        ...
        if ($order) {
            ...
            if (strpos($shippingMethod, 'gls_relay') !== false) {
                if (!empty($relayInformation) && array_search("", $relayInformation) === false) {
                    ...
                    $this->_helperTools->glsLog(__('Error GLS : Can\'t set relay information in order because at least one information is missing in session'),
                        'err');
                }
            }
        }else {
            $this->_helperTools->glsLog(__('Error GLS : Can\'t set relay information in order because can\'t access to order'),
                'err');
        }
    }
}