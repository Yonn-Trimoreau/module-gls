...
class Export extends AbstractModel {
    ...
    public function export($collection, $download = false) {
        if ($collection->getSize() > 0) {
            ...
            foreach ($collection as $order) {
                ...
                // ORDERID
                $aRow['ORDERID'] = $order->getIncrementId();

                // ORDERNAME
                if ($shippingAddress->getCompany()) {
                    $aRow['ORDERNAME'] = mb_strtoupper($shippingAddress->getCompany());
                } else {
                    $aRow['ORDERNAME'] = mb_strtoupper(
                ...
                if (strpos($shippingMethod, 'ls_tohome') !== false && strpos($shippingMethod, 'ls_tohome') !== 0) {
                    if ($international) {
                        $aRow['PRODUCTNO'] = $this->_aProductnoCorrespondance['ls_tohome_international'];
                    } else {
                        $aRow['PRODUCTNO'] = $this->_aProductnoCorrespondance['ls_tohome'];
                    }
                }

                if (strpos($shippingMethod, 'ls_fds') !== false && strpos($shippingMethod, 'ls_fds') !== 0) {
                    if ($international) {
                        $aRow['PRODUCTNO'] = $this->_aProductnoCorrespondance['ls_fds_international'];
                    } else {
                        $aRow['PRODUCTNO'] = $this->_aProductnoCorrespondance['ls_fds'];
                    }
                }

                if (strpos($shippingMethod, 'ls_relay') !== false && strpos($shippingMethod, 'ls_relay') !== 0) {
                    $aRow['PRODUCTNO'] = $this->_aProductnoCorrespondance['ls_relay'];
                }
                if (strpos($shippingMethod, 'ls_express') !== false && strpos($shippingMethod, 'ls_express') !== 0) {
                    $aRow['PRODUCTNO'] = $this->_aProductnoCorrespondance['ls_express'];
                }

                // ORDERWEIGHTTOT
                $totalWeight            = $order->getWeight();
                $aRow['ORDERWEIGHTTOT'] = str_pad(number_format($totalWeight, 2, '.', ''), 5, "0", STR_PAD_LEFT);

                // CONSID
                $aRow['CONSID'] = $order->getCustomerId();

                // CONTACT
                if ($shippingAddress->getCompany()) {
                    $aRow['CONTACT'] = mb_strtoupper(
                        $shippingAddress->getFirstname() . ' ' . $shippingAddress->getLastname(),
                        'UTF-8'
                    );
                } else {
                    if (strpos($shippingMethod, 'ls_express') !== false && strpos($shippingMethod, 'ls_express') !== 0
                        ...
                        $aRow['CONTACT'] = mb_strtoupper($shippingAddress->getFirstname() . ' ' . $shippingAddress->getLastname(), 'UTF-8');
                    }
                }

                // CONTACTMAIL
                $aRow['CONTACTMAIL'] = $shippingAddress->getEmail();

                // CONTACTMOBILE
                $aRow['CONTACTMOBILE'] = $shippingAddress->getTelephone();

                // CONTACTPHONE
                $aRow['CONTACTPHONE'] = $shippingAddress->getTelephone();
                ...
                if (strlen($street[0]) > 35 || strlen($street[1]) > 35 || strlen($street[2]) > 35) {
                    ...
                    $street  = wordwrap($street, 35, ';', false);
                    $aStreet = explode(';', $street);

                    $aRow['STREET1'] = isset($aStreet[0]) ? mb_strtoupper($aStreet[0], 'UTF-8') : '';
                    $aRow['STREET2'] = isset($aStreet[1]) ? mb_strtoupper($aStreet[1], 'UTF-8') : '';
                    $aRow['STREET3'] = isset($aStreet[2]) ? mb_strtoupper($aStreet[2], 'UTF-8') : '';
                } else {
                    // STREET1
                    $aRow['STREET1'] = mb_strtoupper($street[0], 'UTF-8');

                    // STREET2
                    $aRow['STREET2'] = mb_strtoupper($street[1], 'UTF-8');

                    // STREET3
                    $aRow['STREET3'] = mb_strtoupper($street[2], 'UTF-8');
                }

                // COUNTRYCODE
                $aRow['COUNTRYCODE'] = mb_strtoupper($shippingAddress->getCountryId(), 'UTF-8');

                // CITY
                $aRow['CITY'] = mb_strtoupper($shippingAddress->getCity(), 'UTF-8');

                // ZIPCODE
                $aRow['ZIPCODE'] = mb_strtoupper($shippingAddress->getPostcode(), 'UTF-8');

                // REFPR (identifiant du point relais)
                $aRow['REFPR'] = $order->getGlsRelayId();
    ...
    public function printCsv(
        ...
        foreach ($array as $row) {
            ...
            $rowData = '';
            foreach (array_keys($row) as $key) {
                $cellData = $encloser . iconv('UTF-8', 'ISO-8859-9//TRANSLIT', $row[$key]) . $encloser . $delimiter;
                $rowData  .= $cellData;
            }
            $csvData .= $rowData . "\r\n";
        }
